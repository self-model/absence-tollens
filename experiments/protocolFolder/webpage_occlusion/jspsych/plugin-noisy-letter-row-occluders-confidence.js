    ...
    trial(display_element, trial) {
      ...
      var rate_confidence = (confidence) => {
        ...
        p.setup = () => {
          ...
          // determine which rows to hide

          var nrows = this.img.height;
          var number_of_rows_to_hide = Math.round(nrows*trial.hide_proportion)
          trial.hidden_rows = p.shuffle([...Array(nrows).keys()]).slice(0,number_of_rows_to_hide)
          window.hidden_rows = trial.hidden_rows;
        ...
        p.draw = () => {
          if (p.millis()-window.start_time < trial.RT + trial.post_click_delay) {
            ...
            if (trial.descend) {
              window.hidden_rows = window.hidden_rows.map(
                (x)=>{return (x+1)%this.img.height}
              )
            }
            var presented_frame = [];
            for (let y = 0; y < this.img.height; y++) {
              ...
              for (let x = 0; x < this.img.width; x++) {
                p.push()
                if (window.hidden_rows.includes(y)) {
            ...
            for (let i = 0; i<window.hidden_rows.length; i++) {
              let y = window.hidden_rows[i];

              p.push()
              p.fill(trial.hide_color)
              p.translate(p.width/2-trial.pixel_size_factor*this.img.width/2-trial.hide_margins,
                p.height/2+y*trial.pixel_size_factor - (this.img.height/2)*trial.pixel_size_factor)
              p.rect(0,0,trial.pixel_size_factor*this.img.width+trial.hide_margins*2,trial.pixel_size_factor)
              p.pop()
            }
